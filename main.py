...
COMMAND = ["[A]攻撃", "[P]ポーション", "[B]ブレイズジェム", "[R]逃げる"]
TRE_NAME = ["ポーション", "ブレイズジェム", "食料腐敗", "食料値 +20", "食料値 +100"]
EMY_NAME = [
    "不死者の相",
    "クラフトの相",
    "飢餓の相",
...
def init_battle():  # 戦闘に入る準備をする
    ...
    emy_x = 441 - imgEnemy.get_width() / 2
    emy_y = 420 - imgEnemy.get_height()
...
def main():  # メイン処理
    ...
    pygame.init()
    pygame.display.set_caption("Aspects of the Dungeons")
    screen = pygame.display.set_mode((880, 720))
    clock = pygame.time.Clock()
    font = pygame.font.Font('font/mogihaPen.ttf', 40)
    fontS = pygame.font.Font('font/mogihaPen.ttf', 30)
    ...
    while True:
        ...
        if idx == 0:  # タイトル画面
            ...
            if fl_max >= 2:
                draw_text(screen, "最大到達フロア{}.".format(fl_max), 300, 460, font, CYAN)
            draw_text(screen, "スペースキーを押してください", 120, 560, font, BLINK[tmr % 6])
            if key[K_SPACE] == 1:
                make_dungeon()
                put_event()
                floor = 1
            ...
            draw_text(screen, "フロア {} ({},{})".format(floor, pl_x, pl_y), 60, 40, fontS, WHITE)
            if welcome > 0:
                welcome = welcome - 1
                draw_text(screen, "ようこそ フロア{}へ".format(floor), 300, 180, font, CYAN)
            ...
            if tmr <= 30:
                ...
                se[3].play()
                draw_text(screen, "あなたは死んだ", 360, 240, font, RED)
            ...
            if tmr == 1:
                ...
                draw_text(screen, "エネミーと遭遇した！", 350, 200, font, WHITE)
            elif tmr <= 16:
            ...
            draw_battle(screen, fontS)
            if tmr == 1:
                set_message("{}のターン".format(emy_name))
            if tmr == 5:
                set_message(emy_name + "は攻撃を仕掛けてきた！")
                se[0].play()
                emy_step = 30
            if tmr == 9:
                dmg = emy_str + random.randint(0, 9)
                set_message(str(dmg) + "のダメージ！")
                dmg_eff = 5
                emy_step = 0
            if tmr == 15:
                pl_life = pl_life - dmg
            ...
            if tmr == 1:
                ...
                food_p = random.randint(5,10)
            if tmr == 21:
                set_message("最大体力 + " + str(lif_p))
                pl_lifemax = pl_lifemax + lif_p
            if tmr == 26:
                set_message("攻撃力 + " + str(str_p))
                pl_str = pl_str + str_p
            if tmr == 33:
                set_message("食料値 +" + str(food_p))
                food = food + food_p
            if tmr == 50:
                idx = 22
        ...
        draw_text(screen, "[S]移動速度" + str(speed), 640, 40, fontS, WHITE)
